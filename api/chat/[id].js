...
export default async function handler(req, res) {
  if (applyCORS(req, res, { origins: ['http://localhost:5173', 'https://truequeapp.vercel.app'], methods: 'GET,POST,OPTIONS' })) return;
  ...
  if (req.method === "POST") {
    ...
    if (!remitenteId) return res.status(403).json({ error: "Usuario no encontrado" });
